...
export default function AdminPage() {
  ...
  const loadData = async () => {
    const [instRes, reqRes, noticeRes, faqRes] = await Promise.all([
      ...
      supabase.from('faq').select('count', { count: 'exact' }),
    ])
    ...
    <div style={{ background: '#F8FAFC', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '28px 20px' }}>
        ...
        <div style={{ display: 'flex', borderBottom: '0.5px solid rgba(0,0,0,0.1)', marginBottom: '24px' }}>
          <button style={tabStyle('dashboard')} onClick={() => setActiveTab('dashboard')}>대시보드</button>
          <button style={tabStyle('instructors')} onClick={() => setActiveTab('instructors')}>강사 관리</button>
          <button style={tabStyle('requests')} onClick={() => setActiveTab('requests')}>의뢰 관리</button>
          <button style={tabStyle('notices')} onClick={() => setActiveTab('notices')}>공지 관리</button>
        </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {instructors.map(inst => (
                  <tr key={inst.id}>
                    <td style={{ padding: '12px 16px', fontSize: '13px', borderBottom: '0.5px solid rgba(0,0,0,0.07)' }}>
                      {inst.name}
                    </td>
                    ...
                    <td style={{ padding: '12px 16px', borderBottom: '0.5px solid rgba(0,0,0,0.07)' }}>
                      <button onClick={() => handleTogglePremium(inst.id, inst.is_premium)} style={{
                        ...
                        background: inst.is_premium ? '#FEF3C7' : '#F1F5F9',
                        color: inst.is_premium ? '#92400E' : '#64748B'
                      }}>
                    ...
                    <td style={{ padding: '12px 16px', borderBottom: '0.5px solid rgba(0,0,0,0.07)' }}>
                      <span style={{
                        padding: '4px 10px', borderRadius: '6px', fontSize: '11px',
                        fontWeight: '500',
                        background: inst.is_active ? '#DCFCE7' : '#F1F5F9',
                        color: inst.is_active ? '#166534' : '#64748B'
                      }}>
                ...
                {requests.map(req => (
                  <tr key={req.id}>
                    ...
                    <td style={{ padding: '12px 16px', borderBottom: '0.5px solid rgba(0,0,0,0.07)' }}>
                      <span style={{
                        padding: '4px 10px', borderRadius: '6px', fontSize: '11px',
                        fontWeight: '500', background: '#EFF6FF', color: '#2563EB'
                      }}>
                        {req.status || '접수'}
                      </span>
                    </td>
            ...
            <div style={{
              ...
              <div style={{ marginBottom: '12px' }}>
                <input
                  ...
                  style={{
                    width: '100%', border: '0.5px solid rgba(0,0,0,0.18)',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <button onClick={handleNoticeSubmit} style={{
                  ...
                  {notices.map(notice => (
                    <tr key={notice.id}>
                      ...
                      <td style={{ padding: '12px 16px', borderBottom: '0.5px solid rgba(0,0,0,0.07)' }}>
                        <span style={{
                          padding: '4px 10px', borderRadius: '6px', fontSize: '11px',
                          background: notice.is_published ? '#DCFCE7' : '#F1F5F9',
                          color: notice.is_published ? '#166534' : '#64748B'
                        }}>
                      ...
                      <td style={{ padding: '12px 16px', borderBottom: '0.5px solid rgba(0,0,0,0.07)' }}>
                        <button onClick={() => handleDeleteNotice(notice.id)} style={{
